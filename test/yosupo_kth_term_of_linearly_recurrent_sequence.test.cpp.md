...
data:
  ...
  bundledCode: "#line 1 \"test/yosupo_kth_term_of_linearly_recurrent_sequence.test.cpp\"\
    ...
    \ linear_recurrence(const vector<mint> &a, const vector<mint> &c, ll n) {\n  \
    \  if (n < (ll)a.size()) return a[(int)n];\n    int k = (int)c.size();\n    if\
    \ (k == 0) return mint(0);\n    poly q(k + 1);\n    q[0] = 1;\n    for (int i\
    \ = 0; i < k; ++i) q[i + 1] = -c[i];\n    vector<mint> aa(k, mint(0));\n    for\
    \ (int i = 0; i < min(k, (int)a.size()); ++i) aa[i] = a[i];\n    poly p = (poly(aa)\
...
layout: document
redirect_from:
- /verify/test/yosupo_kth_term_of_linearly_recurrent_sequence.test.cpp
- /verify/test/yosupo_kth_term_of_linearly_recurrent_sequence.test.cpp.html
