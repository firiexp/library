...
data:
  ...
  bundledCode: "#line 1 \"test/yosupo_aplusb_wavelet_matrix_top_k_freq.test.cpp\"\n\
    ...
    \ res.end(), [](const pair<int, ll> &x, const pair<int, ll> &y) {\n        if\
    \ (x.first != y.first) return x.first > y.first;\n        return x.second < y.second;\n\
    \    });\n    if ((int)res.size() > k) res.resize(k);\n    return res;\n}\n\n\
    vector<pair<int, int>> brute_top_k_freq_index(const vector<int> &idx, int l, int\
    \ r, int k) {\n    map<int, int> cnt;\n    for (int i = l; i < r; ++i) ++cnt[idx[i]];\n\
    \    vector<pair<int, int>> res;\n    for (const auto &[x, c] : cnt) res.push_back({c,\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/aplusb\"\n\n#include <algorithm>\n\
    ...
    \ auto &[x, c] : cnt) res.push_back({c, x});\n    sort(res.begin(), res.end(),\
    \ [](const pair<int, ll> &x, const pair<int, ll> &y) {\n        if (x.first !=\
    \ y.first) return x.first > y.first;\n        return x.second < y.second;\n  \
    \  });\n    if ((int)res.size() > k) res.resize(k);\n    return res;\n}\n\nvector<pair<int,\
    \ int>> brute_top_k_freq_index(const vector<int> &idx, int l, int r, int k) {\n\
    \    map<int, int> cnt;\n    for (int i = l; i < r; ++i) ++cnt[idx[i]];\n    vector<pair<int,\
    \ int>> res;\n    for (const auto &[x, c] : cnt) res.push_back({c, x});\n    sort(res.begin(),\
...
layout: document
redirect_from:
- /verify/test/yosupo_aplusb_wavelet_matrix_top_k_freq.test.cpp
- /verify/test/yosupo_aplusb_wavelet_matrix_top_k_freq.test.cpp.html
