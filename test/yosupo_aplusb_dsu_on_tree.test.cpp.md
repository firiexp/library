...
data:
  ...
  bundledCode: "#line 1 \"test/yosupo_aplusb_dsu_on_tree.test.cpp\"\n#define PROBLEM\
    ...
    \ n - 1));\n    }\n\n    template<class G>\n    explicit DSUonTree(const vector<vector<G>>\
    \ &g, int root = 0) : DSUonTree((int)g.size()) {\n        vector<int> parent(n,\
    \ -2);\n        vector<int> st(1, root);\n        parent[root] = -1;\n       \
    \ while (!st.empty()) {\n            int v = st.back();\n            st.pop_back();\n\
    \            for (auto &&u : g[v]) {\n                if (u == parent[v] || parent[u]\
    ...
    \n/**\n * @brief DSU on Tree\n */\n#line 18 \"test/yosupo_aplusb_dsu_on_tree.test.cpp\"\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/aplusb\"\n\n#include <cassert>\n\
    ...
    \ \"../tree/dsu_on_tree.cpp\"\n\nbool is_ancestor(const vector<int> &parent, int\
    \ a, int v) {\n    while (v != -1) {\n        if (v == a) return true;\n     \
    \   v = parent[v];\n    }\n    return false;\n}\n\nvector<ll> run_with_clear(const\
    \ DSUonTree &dsu, const vector<ll> &w) {\n    int n = w.size();\n    vector<ll>\
    \ ans(n, -1);\n    ll cur = 0;\n    auto add = [&](int v) {\n        cur += w[v];\n\
    \    };\n    auto query = [&](int v) {\n        ans[v] = cur;\n    };\n    auto\
    \ remove = [&](int v) {\n        cur -= w[v];\n    };\n    dsu.run_add_remove(add,\
    \ query, remove);\n    return ans;\n}\n\nvector<ll> run_with_reset(const DSUonTree\
    \ &dsu, const vector<ll> &w) {\n    vector<ll> ans(w.size(), -1);\n    ll cur\
    \ = 0;\n    auto add = [&](int v) {\n        cur += w[v];\n    };\n    auto query\
    \ = [&](int v) {\n        ans[v] = cur;\n    };\n    auto reset = [&]() {\n  \
    ...
    \        dsu2.build(0);\n\n        for (auto &&dsu : {dsu1, dsu2}) {\n       \
    \     assert(run_with_clear(dsu, w) == want);\n            assert(run_with_reset(dsu,\
    \ w) == want);\n            for (int v = 0; v < n; ++v) {\n                assert(dsu.end(v)\
    \ - dsu.begin(v) == sub_size[v]);\n                for (int u = 0; u < n; ++u)\
    \ {\n                    bool in = dsu.begin(v) <= dsu.idx(u) && dsu.idx(u) <\
    \ dsu.end(v);\n                    assert(in == is_ancestor(parent, v, u));\n\
    \                }\n            }\n        }\n    }\n}\n\nint main() {\n    self_check();\n\
    \n    Scanner sc;\n    Printer pr;\n    ll a, b;\n    sc.read(a, b);\n    pr.println(a\
    \ + b);\n    return 0;\n}\n"
