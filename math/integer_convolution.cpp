#ifndef FIRIEXP_LIBRARY_MATH_INTEGER_CONVOLUTION_CPP
...
namespace BoundedConvolution {
    using NTT1 = ArbitraryConvolution::NTT<998244353, 3>;
    using NTT2 = ArbitraryConvolution::NTT<1004535809, 3>;
    ...
    namespace internal {
        ...
        inline unsigned long long combine_u64(unsigned int x1, unsigned int x2) {
            static constexpr unsigned long long m1 = 998244353ULL;
            ...
            return x1 + m1 * multiplier;
        }
    }

    vector<unsigned long long> convolution_u64(const vector<unsigned int> &a,
        ...
        vector<unsigned long long> res(c1.size());
        for (int i = 0; i < (int)res.size(); ++i) {
            res[i] = internal::combine_u64(c1[i], c2[i]);
        }
        return res;
    }
